...
export default function QuizGame({ student, quiz, onRestart, loading }) {
            ...
            <Fade in timeout={500}>
                <Box sx={{ textAlign: 'center', p: 8 }}>
                    ...
                    <Typography variant="body1" color="text.secondary" component="div">
                        Preparando questões sobre <strong>{student.theme}</strong> em <strong>{student.subject}</strong>
                    </Typography>
                </Box>
            </Fade>
        );
            ...
            <Fade in timeout={500}>
                <Box sx={{ textAlign: 'center', p: 6 }}>
                    ...
                    <Typography variant="body1" color="text.secondary" paragraph>
                        Não foi possível gerar questões. Verifique sua chave de API e tente novamente.
                    </Typography>
                    <Button
                        variant="contained"
                        onClick={onRestart}
    ...
    if (showResult) {
        ...
        const getResultColor = () => {
            if (isExcellent) return 'success.main';
            if (isGood) return 'warning.main';
            return 'error.main';
        };
        const getResultIcon = () => (isExcellent ? '🎉' : isGood ? '👍' : '💪');
        const getResultMessage = () => {
            if (isExcellent) return 'Excelente! Você dominou o conteúdo!';
            if (isGood) return 'Bom Trabalho! Continue assim!';
            ...
            <Fade in timeout={500}>
                <Box sx={{ p: 4 }}>
                    <Box textAlign="center" mb={4}>
                        ...
                        <Typography variant="h4" gutterBottom color={getResultColor()} fontWeight="bold">
                            {getResultMessage()}
                        </Typography>
                    </Box>
                    ...
                    <Grid container spacing={3} mb={4}>
                        {/* ✅ REATORADO: Prop 'item' removida para Grid v2 */}
                        <Grid xs={4}>
                    ...
                    <Box textAlign="center" mb={4}>
                        <Chip icon={<Timer />} label={`Tempo total: ${Math.floor(totalTime / 60)}min ${totalTime % 60}s`} variant="outlined" color="info" />
                    </Box>
                    <Divider sx={{ my: 3 }} />
                    <Stack spacing={2}>
                        <Button
                            ...
                            size="large"
                            sx={{ py: 1.5, borderRadius: 3, fontWeight: 600, background: 'linear-gradient(135deg, #2563eb 0%, #7c3aed 100%)', '&:hover': { transform: 'translateY(-2px)', boxShadow: 4, }, transition: 'all 0.2s ease-in-out' }}
                        >
                            Fazer Outro Quiz
                        </Button>
                        <Button variant="outlined" onClick={onRestart} startIcon={<Home />} size="large" sx={{ py: 1.5, borderRadius: 3 }}>
                            Voltar ao Início
                        </Button>
                    </Stack>
                </Box>
    ...
    return (
        <Fade in timeout={300}>
            <Box sx={{ p: 3 }}>
                ...
                <Grid container spacing={2} mb={3}>
                    {question.options.map((option, index) => {
                        ...
                        if (selectedAnswer !== null) {
                            if (isSelected) {
                                cardStyle = isCorrect
                                    ? { bgcolor: 'success.50', borderColor: 'success.main', transform: 'scale(1.02)' }
                                    : { bgcolor: 'error.50', borderColor: 'error.main', transform: 'scale(1.02)' };
                            } else if (isCorrect) {
                        ...
                        return (
                            // ✅ REATORADO: Prop 'item' removida para Grid v2 e breakpoints ajustados
                ...
                {selectedAnswer !== null && (
                    <Fade in timeout={500}>
                        <Paper sx={{ p: 2, mb: 3, bgcolor: selectedAnswer === question.correctAnswer ? 'success.50' : 'error.50', border: 1, borderColor: selectedAnswer === question.correctAnswer ? 'success.main' : 'error.main' }}>
                            <Stack direction="row" alignItems="center" spacing={1}>
                                {selectedAnswer === question.correctAnswer ? (
                                    <>
                                        <CheckCircle color="success" />
                                        <Typography variant="body1" color="success.main" fontWeight="medium">Resposta correta! Parabéns! 🎉</Typography>
                                    </>
                                ) : (
                                    <>
                                        <Cancel color="error" />
                                        <Typography variant="body1" color="error.main" fontWeight="medium">Resposta incorreta. Continue estudando! 📚</Typography>
                                    </>
                                )}
                            </Stack>
                        </Paper>
                ...
                <Stack direction="row" justifyContent="space-between" alignItems="center">
                    <Button onClick={onRestart} variant="text" startIcon={<RestartAlt />} color="inherit">
                        Voltar ao Início
                    </Button>
                    <Chip label={`${score} acertos`} color="primary" variant="outlined" />
                </Stack>
            </Box>
        </Fade>
    );
